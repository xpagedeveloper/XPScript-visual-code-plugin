...
export class XPScriptDebugAdapter implements vscode.DebugAdapter {
  ...
  private async handleRequest(request:any):Promise<void> {
    switch(request.command) {
      ...
      case 'setBreakpoints': {
        const source=request.arguments?.source?.path??''; const requested=(request.arguments?.breakpoints??[]) as Array<{line:number}>;
